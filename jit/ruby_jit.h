...
{
  jit_context->_rb_out_of_int(num);
}

VALUE rb_int2big(SIGNED_VALUE v)
{
  return jit_context->_rb_int2big(v);
}
...
static
VALUE jit_vm_call_iseq_setup_normal(rb_thread_t *th,
                                    rb_control_frame_t *cfp, CALL_INFO ci, int argc)
{
  int i, local_size;
...
static inline int
jit_block_proc_is_lambda(const VALUE procval)
{
  rb_proc_t *proc;
...
static int
jit_vm_yield_setup_block_args(rb_thread_t *th, const rb_iseq_t * iseq,
...
static inline int
jit_vm_yield_setup_args(rb_thread_t * const th, const rb_iseq_t *iseq,
